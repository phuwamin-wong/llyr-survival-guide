# Hyperkalemia

## Definition

* Serum potassium ≥ 5.5 mmol/L
* Severity grading
  * Mild = 5.5 - 6.0 mmol/L
  * Moderate = 6.1 - 7.0 mmol/L
  * Severe > 7.0 mmol/L

## Patient evaluation

* Rule-out emergency condition
  * Arrhythmia
    * EKG changes หากมีถือว่าเป็น Emergency\*\*\*
      * Tall peaked T
      * Flattened T wave, prolonged PR interval, depressed ST segment
      * Atrial standstill
      * Prolonged QRS duration
      * Sinusoid pattern

<figure><img src="../../.gitbook/assets/image (1).png" alt=""><figcaption></figcaption></figure>

## Approach

* **Rule-out** pseudohyperkalemia: Prolonged tourniquet time, hemolysis, WBC > 2

{% hint style="info" %}
ถ้า Hemolysis ≥ 2+ -- > repeat lab Electrolyte
{% endhint %}

* **Find** causes of **Extracellular shift of K**
  * **Propranolol, Digitalis, Succinylcholine**
* **True hyperkalemia**
  * GFR ≤ 20 --> AKI, CKD
  * GFR ≥ 20
    * **High K intake**
    * **Hyperkalemic RTA (type IV RTA)**
    * Primary hypoaldosteronism (Addison's disease)
    * Secondary hypoaldosteronism (hyporeninemic hypoaldosteronism)
      * DM
      * Obstructive uropathy
      * Tubulointerstitial disease
      * SLE
      * HIV
    * Drugs:
      * NSAIDs, ACEi/ARB, Heparin
      * Spironolactone, Amiloride, Triamterene, TMP/SMX

## Management

* Should EKG 12 leads (esp ≥ 6 mmol/L)
* **EKG change?**
  * 10% Calcium gluconate 10 mL IV drip in 10 min
* Lower K in serum
  * **Intracellular shift of K**
    * RI 10 unit + Glucose 50 mL IV slowly push (if POCT-glucose < 250 mg%)
    * Salbutamol 4 NB (10 mg) NB stat
  * **GI removal of K**
    * Kalimate 30 g + น้ำ 50 mL PO q 4 hr x 1 - 3 doses (ขึ้นกับ degree of hyperkalemia)
    * หากกินไม่ได้: Kalimate 30 g + น้ำ 300 mL RS q 4 hr x 1 - 3 doses (ขึ้นกับ degree of hyperkalemia)
  * **Renal removal of K**
    * Furosemide starting dose ขึ้นกับ creatinine level
  * **Hemolysis**

## Example Order - NPO

|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |                                                                                                                                                                                                                                                                                                                     |   |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | - |
| <p>20/4/2569 21:00<br>Notify K = 6.2 mmol/L<br>Case female 62-year-old<br>#CKD stage 4A2 from DKD<br>#AKI on top CKD from NSAIDs<br>#UGIH S/P EGG --> Clean-based GU<br><br><strong>S:</strong> ตื่น ไม่เหนื่อย ขาไม่บวม ปัสสาวะยังออกอยู่<br><strong>NPO อยู่</strong></p><p>I/O balance to positive<br><br>Last Lab<br>BUN 87 Cr 1.8</p><p>Na 131 K 6.2 Cl 109 CO2 20<br><strong>O</strong>: V/S stable<br>CVS: regular, no murmur.<br>Neuro: Alert and aware<br>Ext: Pitting edema 1+<br><strong>A:</strong> Hyperkalemia from AKI on top CKD<br><strong>P:</strong> Lower K</p> | <ul><li>EKG 12 leads --> tall peaked T</li><li>POCT-glucose stat = 127 mg%</li><li>10% Calcium gluconate 10 mL IV drip in 10 min</li><li>RI 10 U + Glucose 50 mL IV slowly push</li><li>Furosemide 40 mg IV stat</li><li>Kalimate 30 g + น้ำ 200 mL RS q 4 hr x 2 doses</li><li>Electrolyte หลังแก้ K ครบ</li></ul> |   |
